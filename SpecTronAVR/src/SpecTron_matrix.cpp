...
SpecTron_Matrix::SpecTron_Matrix() 
{
    //data = _data;
    //load = _load;
    //clock = _clock;
...
void SpecTron_Matrix::init(byte _data, byte _load, byte _clock, byte _num, int _rotation)
{
  data = _data;
  load = _load;
  clock = _clock;
...
void SpecTron_Matrix::setIntensity(byte intensity)
{
    setCommand(max7219_reg_intensity, intensity);
}

void SpecTron_Matrix::clearMatrix()
{
    for (int i=0; i<8; i++) 
        setColumnAll(i,0);
...
void SpecTron_Matrix::setCommand(byte command, byte value)
{
#if defined(ESP32)
...
void SpecTron_Matrix::setColumn(byte col, byte value)
{
    int n = col / 8;
    int c = col % 8;
#if defined(ESP32)
...
void SpecTron_Matrix::setColumnAll(byte col, byte value)
{
#if defined(ESP32)
    for (int i=0; i<num; i++) 
    {
...
void SpecTron_Matrix::setDot(byte col, byte row, byte value)
{
...
// routine for SpecTron and ZOWI, for the 6 x 5 matrix
void SpecTron_Matrix::writeFull(unsigned long value) {
  if (rotation == 1) {
    for (int r=0; r<5;r++){
            for (int c=0; c<6; c++){
...
void SpecTron_Matrix::sendChar (const byte data, byte pos, byte number, byte scrollspeed){
...
// we need to display first character and scroll left until each charater is shown.
    // we need to display first character and scroll left until each character is shown.
    ...
    // Display "SpecTron Welcome"
    const char* message = "SpecTron Welcome";
    int messageLength = strlen(message);

    for (int charIndex = 0; charIndex < messageLength; charIndex++) {
        char currentChar = message[charIndex];
        if (currentChar >= 32 && currentChar <= 126) { // only displayable characters
            sendChar(currentChar - 32, charIndex, messageLength, 100); // Adjust scrollspeed as needed
        }
    }
}
