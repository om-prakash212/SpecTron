...
#endif
#include "SpecTronAVR.h"
#include <Oscillator.h>

void SpecTron::init(int YL, int YR, int RL, int RR, bool load_calibration, int Buzzer) {
...
///////////////////////////////////////////////////////
void SpecTron::initMATRIX(int DIN, int CS, int CLK, int rotate){
ledmatrix.init( DIN, CS, CLK, 1, rotate);   // set up Matrix display
}
void SpecTron::matrixIntensity(int intensity){
...
//-- OSCILLATORS TRIMS ------------------------------------------//
///////////////////////////////////////////////////////////////////
void SpecTron::setTrims(int YL, int YR, int RL, int RR) {
  servo[0].SetTrim(YL);
  servo[1].SetTrim(YR);
  servo[2].SetTrim(RL);
  servo[3].SetTrim(RR);
...
void SpecTron::saveTrimsOnEEPROM() {
...
void SpecTron::_moveSingle(int position, int servo_number) {
...
void SpecTron::oscillateServos(int A[4], int O[4], int T, double phase_diff[4], float cycle=1){
...
void SpecTron::_execute(int A[4], int O[4], int T, double phase_diff[4], float steps = 1.0){
...
void SpecTron::home(){
  ...
  if(isSpecTronResting==false){ //Go to rest position only if necessary
...
///////////////////////////////////////////////////////////////////
//-- SpecTron movement: Jump
//--  Parameters:
//--    steps: Number of steps
//--    T: Period
//---------------------------------------------------------
void SpecTron::jump(float steps, int T){
...
//---------------------------------------------------------
//-- SpecTron gait: Walking  (forward or backward)
//--  Parameters:
//--    * steps:  Number of steps
//--    * T : Period
...
//---------------------------------------------------------
//-- SpecTron gait: Turning (left or right)
//--  Parameters:
//--   * Steps: Number of steps
//--   * T: Period
//--   * Dir: Direction: LEFT / RIGHT
//---------------------------------------------------------
void SpecTron::turn(float steps, int T, int dir){

  //-- Same coordination than for walking (see SpecTron::walk)
...
//---------------------------------------------------------
//-- SpecTron gait: Lateral bend
//--  Parameters:
//--    steps: Number of bends
//--    T: Period of one bend
//--    dir: RIGHT=Right bend LEFT=Left bend
//---------------------------------------------------------
void SpecTron::bend (int steps, int T, int dir){
  ...
  {
    bend1[2]=180-35;
    bend1[3]=180-60;  //Not 65. SpecTron is unbalanced
    bend2[2]=180-105;
    bend2[3]=180-60;
  }
...
//---------------------------------------------------------
//-- SpecTron gait: Shake a leg
//--  Parameters:
//--    steps: Number of shakes
//--    T: Period of one shake
//--    dir: RIGHT=Right leg LEFT=Left leg
//---------------------------------------------------------
void SpecTron::shakeLeg (int steps,int T,int dir){
...
//---------------------------------------------------------
void SpecTron::updown(float steps, int T, int h){
...
//---------------------------------------------------------
//-- SpecTron movement: swinging side to side
//--  Parameters:
//--     steps: Number of steps
//--     T : Period
//--     h : Amount of swing (from 0 to 50 aprox)
//---------------------------------------------------------
void SpecTron::swing(float steps, int T, int h){
...
//---------------------------------------------------------
//-- SpecTron movement: swinging side to side without touching the floor with the heel
//--  Parameters:
//--     steps: Number of steps
//--     T : Period
//--     h : Amount of swing (from 0 to 50 aprox)
//---------------------------------------------------------
void SpecTron::tiptoeSwing(float steps, int T, int h){
...
//---------------------------------------------------------
//-- SpecTron gait: Jitter
//--  Parameters:
//--    steps: Number of jitters
//--    T: Period of one jitter
//--    h: height (Values between 5 - 25)
//---------------------------------------------------------
void SpecTron::jitter(float steps, int T, int h){
...
//---------------------------------------------------------
//-- SpecTron gait: Ascending & turn (Jitter while up&down)
//--  Parameters:
//--    steps: Number of bends
//--    T: Period of one bend
//--    h: height (Values between 5 - 15)
//---------------------------------------------------------
void SpecTron::ascendingTurn(float steps, int T, int h){
...
//---------------------------------------------------------
//-- SpecTron gait: Moonwalker. SpecTron moves like Michael Jackson
//--  Parameters:
//--    Steps: Number of steps
//--    T: Period
...
//--    dir: Direction: LEFT / RIGHT
//---------------------------------------------------------
void SpecTron::moonwalker(float steps, int T, int h, int dir){

  //-- This motion is similar to that of the caterpillar robots: A travelling
  //-- wave moving from one side to another
  //-- The two SpecTron's feet are equivalent to a minimal configuration. It is known
  //-- that 2 servos can move like a worm if they are 120 degrees out of phase
  //-- In the example of SpecTron, the two feet are mirrored so that we have:
...
//----------------------------------------------------------
//-- SpecTron gait: Crusaito. A mixture between moonwalker and walk
//--   Parameters:
//--     steps: Number of steps
//--     T: Period
...
//--     dir:  Direction: LEFT / RIGHT
//-----------------------------------------------------------
void SpecTron::crusaito(float steps, int T, int h, int dir){
...
//---------------------------------------------------------
void SpecTron::flapping(float steps, int T, int h, int dir){
...
//-- MOUTHS & ANIMATIONS ----------------------------------------//
///////////////////////////////////////////////////////////////////
void SpecTron::setLed(byte X, byte Y, byte value){
  ledmatrix.setDot( X,  Y, value);
}


// EXAMPLE putAnimationMouth(dreamMouth,0);
void SpecTron::putAnimationMouth(unsigned long int aniMouth, int index){
...
void SpecTron::putMouth(unsigned long int mouth, bool predefined){
  if (predefined){
...
void SpecTron::clearMouth(){

  ledmatrix.clearMatrix();
}

void SpecTron::writeText(const char * s, byte scrollspeed){
 int a ;
 int b ;
  for(a = 0; s[a] != '\0'; a++){
    b = a +1 ;
...
void SpecTron::_tone (float noteFrequency, long noteDuration, int silentDuration){
    ...
    // delay(500);
      ...
      tone(SpecTron::pinBuzzer, noteFrequency, noteDuration);
      delay(noteDuration);       //milliseconds to microseconds
      //noTone(PIN_Buzzer);
      delay(silentDuration);
}

void SpecTron::bendTones (float initFrequency, float finalFrequency, float prop, long noteDuration, int silentDuration){
...
void SpecTron::sing(int songName){
  switch(songName){
...
void SpecTron::playGesture(int gesture){
...
void SpecTron::enableServoLimit(int diff_limit) {
  for (int i = 0; i < 4; i++) {
    servo[i].SetLimiter(diff_limit);
  }
}

void SpecTron::disableServoLimit() {
  for (int i = 0; i < 4; i++) {
